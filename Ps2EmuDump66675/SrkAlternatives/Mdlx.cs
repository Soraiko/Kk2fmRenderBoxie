	...
	public class Mdlx : Bar
	{
		...
		public void ExportDAE(string filename)
		{
			...
			for (int h = 0; h < this.models.Count; h++)
			{
				...
				int textureCount = this.models[h].Textures.Count;
				...
				for (int k = 0; k < this.models[h].Skeleton.Joints.Count; k++)
				{
					...
					matrix_node.InnerText += this.models[h].Skeleton.Joints[k].Transform.M11.ToString("0.000000") + " ";
					matrix_node.InnerText += this.models[h].Skeleton.Joints[k].Transform.M21.ToString("0.000000") + " ";
					matrix_node.InnerText += this.models[h].Skeleton.Joints[k].Transform.M31.ToString("0.000000") + " ";
					matrix_node.InnerText += this.models[h].Skeleton.Joints[k].Transform.M41.ToString("0.000000") + " ";
					matrix_node.InnerText += this.models[h].Skeleton.Joints[k].Transform.M12.ToString("0.000000") + " ";
					matrix_node.InnerText += this.models[h].Skeleton.Joints[k].Transform.M22.ToString("0.000000") + " ";
					matrix_node.InnerText += this.models[h].Skeleton.Joints[k].Transform.M32.ToString("0.000000") + " ";
					matrix_node.InnerText += this.models[h].Skeleton.Joints[k].Transform.M42.ToString("0.000000") + " ";
					matrix_node.InnerText += this.models[h].Skeleton.Joints[k].Transform.M13.ToString("0.000000") + " ";
					matrix_node.InnerText += this.models[h].Skeleton.Joints[k].Transform.M23.ToString("0.000000") + " ";
					matrix_node.InnerText += this.models[h].Skeleton.Joints[k].Transform.M33.ToString("0.000000") + " ";
					matrix_node.InnerText += this.models[h].Skeleton.Joints[k].Transform.M43.ToString("0.000000") + " ";
					matrix_node.InnerText += this.models[h].Skeleton.Joints[k].Transform.M14.ToString("0.000000") + " ";
					matrix_node.InnerText += this.models[h].Skeleton.Joints[k].Transform.M24.ToString("0.000000") + " ";
					matrix_node.InnerText += this.models[h].Skeleton.Joints[k].Transform.M34.ToString("0.000000") + " ";
					matrix_node.InnerText += this.models[h].Skeleton.Joints[k].Transform.M44.ToString("0.000000");
				...
				for (int j = 0; j < this.models[h].Meshes.Count + this.models[h].ShadowMeshes.Count; j++)
				{
					...
					for (int k = 0; k < mesh.reverseVertices.Count; k++)
					{
						...
						for (int l = 0; l < mesh.reverseVertices[k].Length; l++)
						{
							...
							if (this.models[h].Skeleton != null)
								v4 = Vector4.Transform(v4, this.models[h].Skeleton.Joints[mesh.influences[k][l]].ComputedTransform);
							v.X += v4.X;
							v.Y += v4.Y;
							v.Z += v4.Z;
						}
					...
					if (mesh.influences.Count>0)
					{
						...
						for (int k = 0; k < this.models[h].Skeleton.Joints.Count; k++)
						{
							Matrix4 m = Matrix4.Invert(this.models[h].Skeleton.Joints[k].ComputedTransform);
		...
		void GetModelData()
		{
			for (int i = 0; i < this.Files.Count; i++)
			{
					...
					this.Files[i].Type == 4)
				{
					...
					{
						model.WrapModes = new List<OpenTK.Graphics.OpenGL.TextureWrapMode[]>(0);
						var listBitmapWrapModes = KenunoTim.GetTextures(this.Files[i + 1].Data);

						for (int w=0;w< listBitmapWrapModes.Count;w++)
						{
							Bitmap bmp = listBitmapWrapModes[w].pic;
							var addressMode = listBitmapWrapModes[w].TextureAddressMode;

							int left = addressMode.Left;
							int right = addressMode.Right;
							int top = addressMode.Top;
							int bottom = addressMode.Bottom;
							if (right < left)
							{
								left = addressMode.Right;
								right = addressMode.Left;
							}
							if (bottom < top)
							{
								top = addressMode.Bottom;
								bottom = addressMode.Top;
							}
							...
							TextureBrush myTBrush = new TextureBrush(bmp, rect, imageAttr);
							myTBrush.TranslateTransform((int)rect.X, (int)rect.Y);

							if (addressMode.AddressU == KenunoTim.TextureWrapMode.RegionRepeat &&
								addressMode.AddressV != KenunoTim.TextureWrapMode.RegionRepeat)
							{
								gr.FillRectangle(myTBrush, rect.X, rect.Y, bmp.Width, rect.Height);
								gr.FillRectangle(myTBrush, rect.X - bmp.Width, rect.Y, bmp.Width, rect.Height);
							}
							else
							if (addressMode.AddressU != KenunoTim.TextureWrapMode.RegionRepeat &&
								addressMode.AddressV == KenunoTim.TextureWrapMode.RegionRepeat)
							{
								gr.FillRectangle(myTBrush, rect.X, rect.Y, rect.Width, bmp.Height);
								gr.FillRectangle(myTBrush, rect.X, rect.Y - bmp.Height, rect.Width, bmp.Height);
							}
							else
							if (addressMode.AddressU == KenunoTim.TextureWrapMode.RegionRepeat &&
								addressMode.AddressV == KenunoTim.TextureWrapMode.RegionRepeat)
							{
								gr.FillRectangle(myTBrush, rect.X, rect.Y, bmp.Width, bmp.Height);
								gr.FillRectangle(myTBrush, rect.X - bmp.Width, rect.Y, bmp.Width, bmp.Height);
								gr.FillRectangle(myTBrush, rect.X + bmp.Width, rect.Y, bmp.Width, bmp.Height);

								gr.FillRectangle(myTBrush, rect.X, rect.Y - bmp.Height, bmp.Width, bmp.Height);
								gr.FillRectangle(myTBrush, rect.X, rect.Y + bmp.Height, bmp.Width, bmp.Height);
							}
							else
							{
								gr.DrawImage(bmp, rect, rect, GraphicsUnit.Pixel);
								if (addressMode.AddressU == KenunoTim.TextureWrapMode.RegionClamp)
								{
									gr.DrawImage(bmp, new RectangleF(0, rect.Y, rect.X, rect.Height), new RectangleF(rect.X, rect.Y, 0.1f, rect.Height), GraphicsUnit.Pixel);
									gr.DrawImage(bmp, new RectangleF(rect.X + rect.Width, rect.Y, bmp.Width - rect.X - rect.Width, rect.Height), new RectangleF(rect.X + rect.Width - 1, rect.Y, 0.1f, rect.Height), GraphicsUnit.Pixel);
								}
								if (addressMode.AddressU == KenunoTim.TextureWrapMode.RegionClamp)
								{
									gr.DrawImage(bmp, new RectangleF(rect.X, 0, rect.Width, rect.Y), new RectangleF(rect.X, rect.Y, rect.Width, 0.1f), GraphicsUnit.Pixel);
									gr.DrawImage(bmp, new RectangleF(rect.X, rect.Y + rect.Height, rect.Width, bmp.Height - rect.Y - rect.Height), new RectangleF(rect.X, rect.Y + rect.Height - 1, rect.Width, 0.1f), GraphicsUnit.Pixel);
								}
							}
		...
		public class Model : BitConverter
		{
			...
			{
				List<short> all_influences = new List<short>(0);
				...
				for (int i = 0; i < count_per_mat_verts; i++)
				{
					...
					for (int j = 0; j < cnt; j++)
						all_influences.Add(this.Int16(mati_cursor));
					mati_cursor += 4;
				}
				mati_cursor += 4;
				...
				if (count_influences == 0)
				{
					for (int i = 0; i < count_vertices; i++)
					{
						...
						mesh.influences.Add(new short[] { all_influences[i] });
					}
				}
				else
				{
