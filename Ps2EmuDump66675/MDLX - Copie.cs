...
namespace BDxGraphiK
{
	public class MDLX
	{
		...
		public class RAM_Model
		{
			...
			public const int PLAY_ACTIVITY_PTR = 0x00349E1C;
			...
			public Vector3 RootTranslate = new Vector3(Single.NaN);
			...
			public void UpdateWithMemregionData(Object3D model)
			{
				...
				double readDouble = UpAxises.Y * System.BitConverter.ToSingle(this.MemRegionData, 0x55C) + MathHelper.Pi;
				...
				Vector3 worldPos = UpAxises * new Vector3(System.BitConverter.ToSingle(this.MemRegionData, 0x540), System.BitConverter.ToSingle(this.MemRegionData, 0x544), System.BitConverter.ToSingle(this.MemRegionData, 0x548));
				...
				Vector3 rootTranslate = Vector3.Zero;
				Vector3 worldTranShift = Vector3.Zero;
				...
				if (ingame)
				{
					...
					{
						model.Skeleton.Joints[m].ComputedTransform = matrices[m];
					}
					model.Skeleton.ReverseComputedMatrices();

					rootTranslate = model.Skeleton.Joints[0].Transform.ExtractTranslation();

					if (positionInfo_hasCurveIndex[4])
					{
						worldRotY = this.WorldRotationY;
					}


					for (int i = 0; i < 3; i++)
						if (positionInfo_hasCurveIndex[6 + i])
						{
							this.RootTranslate[i] = rootTranslate[i];
							worldTranShift[i] = -this.RootTranslate[i];
						}
						else
						{
							if (Single.IsNaN(this.RootTranslate[i]) == false && Math.Abs(this.RootTranslate[i] - rootTranslate[i]) < 0.01)
								worldTranShift[i] = -this.RootTranslate[i];
							else
								this.RootTranslate[i] = Single.NaN;
						}




					model.Skeleton.Transform = Matrix4.CreateTranslation(worldTranShift) * Matrix4.CreateRotationY(worldRotY) * Matrix4.CreateTranslation(worldPos);
				...
				{
					rootTranslate = matrices[0].ExtractTranslation();

					for (int i = 0; i < 3; i++)
					if (hasOffset||Math.Abs(positionInfo_scale_rot_trans[2][i]) > 0)
					{
						worldTranShift[i] = -rootTranslate[i];
					}
					...
					for (int m = 0; m < this.BonesCount; m++)
					{
						model.Skeleton.Joints[m].ComputedTransform = matrices[m] * model.Skeleton.Transform;
					}
					model.Skeleton.PassComputedTransforms();
					model.Skeleton.SendMatricesToUniformObject();
				}
				...
				float opacity = System.BitConverter.ToSingle(this.MemRegionData, 0x834) * System.BitConverter.ToSingle(this.MemRegionData, 0x82C);
