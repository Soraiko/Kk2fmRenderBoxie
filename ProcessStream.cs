...
namespace OpenKh
{
	public class ProcessStream : Stream
	{
		...
		[DllImport("kernel32.dll")]
		static extern bool VirtualProtectEx(IntPtr hProcess, long lpAddress, UIntPtr dwSize, uint flNewProtect, out uint lpflOldProtect);
		...
		public override int Read(byte[] buffer, int offset, int count)
		{
			...
			if (offset == 0)
			{
				ReadProcessMemory(hProcess, BaseOffset+pos, buffer, count, out read);
			}
			else
			{
		...
		public override void Write(byte[] buffer, int offset, int count)
		{
			...
			if (offset == 0)
			{
				WriteProcessMemory(hProcess, BaseOffset + pos, buffer, count, out written);
			}
			else
			{
				var data = new byte[count];
				Array.Copy(buffer, offset, data, 0, count);
				WriteProcessMemory(hProcess, BaseOffset + pos, data, count, out written);
			}
