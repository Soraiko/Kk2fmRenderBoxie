...
namespace BDxGraphiK
{
	public class Frustrum
	{
		...
		void NormalizePlane(ref float[][] frustum, int side)
		{
		...
		public void CalculateFrustum()
		{
			...
			m_Frustum[(int)FrustumSide.RIGHT][(int)PlaneData.A] = clip[3] - clip[0];
			m_Frustum[(int)FrustumSide.RIGHT][(int)PlaneData.B] = clip[7] - clip[4];
			m_Frustum[(int)FrustumSide.RIGHT][(int)PlaneData.C] = clip[11] - clip[8];
			m_Frustum[(int)FrustumSide.RIGHT][(int)PlaneData.D] = clip[15] - clip[12];

			// Now that we have a normal (A,B,C) and a distance (D) to the plane,
			// we want to normalize that normal and distance.

			// Normalize the RIGHT side
			NormalizePlane(ref m_Frustum, (int)FrustumSide.RIGHT);

			// This will extract the LEFT side of the frustum
			m_Frustum[(int)FrustumSide.LEFT][(int)PlaneData.A] = clip[3] + clip[0];
			m_Frustum[(int)FrustumSide.LEFT][(int)PlaneData.B] = clip[7] + clip[4];
			m_Frustum[(int)FrustumSide.LEFT][(int)PlaneData.C] = clip[11] + clip[8];
			m_Frustum[(int)FrustumSide.LEFT][(int)PlaneData.D] = clip[15] + clip[12];

			// Normalize the LEFT side
			NormalizePlane(ref m_Frustum, (int)FrustumSide.LEFT);

			// This will extract the BOTTOM side of the frustum
			m_Frustum[(int)FrustumSide.BOTTOM][(int)PlaneData.A] = clip[3] + clip[1];
			m_Frustum[(int)FrustumSide.BOTTOM][(int)PlaneData.B] = clip[7] + clip[5];
			m_Frustum[(int)FrustumSide.BOTTOM][(int)PlaneData.C] = clip[11] + clip[9];
			m_Frustum[(int)FrustumSide.BOTTOM][(int)PlaneData.D] = clip[15] + clip[13];

			// Normalize the BOTTOM side
			NormalizePlane(ref m_Frustum, (int)FrustumSide.BOTTOM);

			// This will extract the TOP side of the frustum
			m_Frustum[(int)FrustumSide.TOP][(int)PlaneData.A] = clip[3] - clip[1];
			m_Frustum[(int)FrustumSide.TOP][(int)PlaneData.B] = clip[7] - clip[5];
			m_Frustum[(int)FrustumSide.TOP][(int)PlaneData.C] = clip[11] - clip[9];
			m_Frustum[(int)FrustumSide.TOP][(int)PlaneData.D] = clip[15] - clip[13];

			// Normalize the TOP side
			NormalizePlane(ref m_Frustum, (int)FrustumSide.TOP);

			// This will extract the BACK side of the frustum
			m_Frustum[(int)FrustumSide.BACK][(int)PlaneData.A] = clip[3] - clip[2];
			m_Frustum[(int)FrustumSide.BACK][(int)PlaneData.B] = clip[7] - clip[6];
			m_Frustum[(int)FrustumSide.BACK][(int)PlaneData.C] = clip[11] - clip[10];
			m_Frustum[(int)FrustumSide.BACK][(int)PlaneData.D] = clip[15] - clip[14];

			// Normalize the BACK side
			NormalizePlane(ref m_Frustum, (int)FrustumSide.BACK);

			// This will extract the FRONT side of the frustum
			m_Frustum[(int)FrustumSide.FRONT][(int)PlaneData.A] = clip[3] + clip[2];
			m_Frustum[(int)FrustumSide.FRONT][(int)PlaneData.B] = clip[7] + clip[6];
			m_Frustum[(int)FrustumSide.FRONT][(int)PlaneData.C] = clip[11] + clip[10];
			m_Frustum[(int)FrustumSide.FRONT][(int)PlaneData.D] = clip[15] + clip[14];

			// Normalize the FRONT side
			NormalizePlane(ref m_Frustum, (int)FrustumSide.FRONT);
		}
		...
		public bool PointInFrustum(float x, float y, float z)
		{
			...
			for (int i = 0; i < 6; i++)
			{
				...
				if (m_Frustum[i][(int)PlaneData.A] * x + m_Frustum[i][(int)PlaneData.B] * y + m_Frustum[i][(int)PlaneData.C] * z + m_Frustum[i][(int)PlaneData.D] <= 0)
				{
		...
		public bool SphereInFrustum(Vector4 sphere)
		{
			...
			for (int i = 0; i < 6; i++)
			{
				...
				if (m_Frustum[i][(int)PlaneData.A] * sphere.X + m_Frustum[i][(int)PlaneData.B] * sphere.Y + m_Frustum[i][(int)PlaneData.C] * sphere.Z + m_Frustum[i][(int)PlaneData.D] <= -sphere.W)
				{
		...
		public bool CubeInFrustum(float x, float y, float z, float size)
		{
			...
			for (int i = 0; i < 6; i++)
			{
				if (m_Frustum[i][(int)PlaneData.A] * (x - size) + m_Frustum[i][(int)PlaneData.B] * (y - size) + m_Frustum[i][(int)PlaneData.C] * (z - size) + m_Frustum[i][(int)PlaneData.D] > 0)
					continue;
				if (m_Frustum[i][(int)PlaneData.A] * (x + size) + m_Frustum[i][(int)PlaneData.B] * (y - size) + m_Frustum[i][(int)PlaneData.C] * (z - size) + m_Frustum[i][(int)PlaneData.D] > 0)
					continue;
				if (m_Frustum[i][(int)PlaneData.A] * (x - size) + m_Frustum[i][(int)PlaneData.B] * (y + size) + m_Frustum[i][(int)PlaneData.C] * (z - size) + m_Frustum[i][(int)PlaneData.D] > 0)
